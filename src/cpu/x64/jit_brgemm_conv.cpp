    ...
    parallel(jcp.nthr, [&](const int ithr, const int nthr) {
        ...
        if (jcp.loop_order == loop_ndhwgc)
            ...
            nd_iterator_init(start, n, jcp.mb, g, jcp.ngroups, ocb, jcp.nb_oc,
                    odb, jcp.nb_od, owb, jcp.nb_ow, ohb, jcp.nb_oh);
        else
        ...
        int last_ohb = -1;
        int last_owb = -1;
        for (auto work = start; work < end;) {
            for (int icc = 0; icc < ic_chunks; icc++) {
                for (int ohb_ = 0; ohb_ < jcp.nb_oh; ohb_++) {
                    btc.g = g;
                    btc.n = n;
                    btc.ocb = ocb;
                    btc.odb = odb;
                    ...
                    auto od_begin = odb * jcp.od_block;
                    auto od_end = nstl::min(OD, od_begin + jcp.od_block);
                    auto oh_begin = ohb_ * jcp.oh_block;
                        ...
                        for (int oh = oh_begin; oh < oh_end; oh++) {
                            {
                                ...
                                if (jcp.exec_type == exec_base) {
                                    ...
                                    maybe_conv_inp(ithr, src, inp_buffer, inp_buffer_mask,
                                        g, n, icc, odb, ohb_, owb, last_g, last_n,
            ...
            if (jcp.loop_order == loop_ndhwgc) {
                ...
                nd_iterator_jump(work, end, n, jcp.mb, g, jcp.ngroups, ocb, jcp.nb_oc, odb,
                    jcp.nb_od, owb, jcp.nb_ow, ohb, jcp.nb_oh);
            }
            else
                assert(!"Unknown loop order");
        }
