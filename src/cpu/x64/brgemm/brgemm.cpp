...
status_t brgemm_desc_set_postops(brgemm_t *brg, const primitive_attr_t *attr,
        ...
        bool is_weights_decompression, const memory_desc_t *wei_md) {
    ...
    if (brg->with_wei_decomp) {
        ...
        if (brg->with_wei_decomp_scales) {
            brg->wei_decomp_scales_stride = wei_scales.dims_[0];
        ...
        if (brg->with_wei_decomp_zero_points) {
            brg->wei_decomp_zero_points_stride = attr->zero_points_.get_dims(DNNL_ARG_WEIGHTS)[0];
            brg->wei_decomp_zero_points_group_size = wei_d.dims()[1] / attr->zero_points_.get_dims(DNNL_ARG_WEIGHTS)[1];
            brg->with_grouped_wei_decomp |= attr->zero_points_.get_dims(DNNL_ARG_WEIGHTS)[0] != 1;
        }
    }
