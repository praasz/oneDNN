...
status_t brgemm_matmul_t<isa>::pd_t::init(engine_t *engine) {
    ...
    bool ok = is_dense_data() && mayiuse(isa) && problem_dt_correct
            && IMPLICATION(is_f16, isa == avx512_core_fp16)
            && !has_zero_dim_memory() && no_dynamic_strides_for_B_and_C
...
status_t brgemm_matmul_t<isa>::init(engine_t *engine) {
    ...
    for (int i_init = 0; i_init < 2; i_init++) {
        ...
        // std::cout << "brg_kernels_" << idx << "|" << brg_kernels_[idx].get() << std::endl;
...
//     std::cout << "M|" << M << "|brg_ker_idx|"  << brg_ker_idx << std::endl;
//     std::cout << "M|" << M << "|is_bs_tail|" << is_bs_tail << "|do_init|" << do_init
//         << "|m_ker_idx|" << m_ker_idx << "|is_N_tail|" << is_N_tail << std::endl;
...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    ...
    dim_t get_data_A_off(int b, int m, int k) const {
        ...
        if (bgmmc_.src_tag == acbd || bgmmc_.src_tag == adbc) {
            ...
            return A_strides_[2] * b + A_strides_[1] * m
                    + A_strides_[0] * k;
        }
    }
