...
// Returns the max divisor of n in range [d_min, d_max].
int max_divisor(int n, int d_min, int d_max) {
    if (n == 0) return 0;
    if (n < 0) n = -n;
    if (d_max <= 0) d_max = n;
    if (d_min <= 0) d_min = 1;
    if (d_max < d_min) nstl::swap(d_max, d_min);

    for (int d = d_max; d >= d_min; d--)
        if (n % d == 0) return d;
    return d_min;
}
...
int get_os_block(const jit_brgemm_primitive_conf_t &jbgp, bool try_to_adjust,
    ...
    if (min_os_block > 0 && max_os_block > 0)
        os_block = max_divisor(jbgp.os, min_os_block, max_os_block);
...
status_t init_ip_conf_fwd(jit_brgemm_primitive_conf_t &jbgp,
    ...
    if (jbgp.os > 256 && is_f32_compute) {
        ...
        jbgp.nb_os_blocking = max_divisor(jbgp.nb_os, 1, jbgp.nb_os_blocking);
    }
    ...
    while (!balanced && jbgp.os_block > min_os_block && is_f32_compute) {
        ...
        jbgp.os_block = max_divisor(jbgp.os, min_os_block, max_os_block);
        jbgp.nb_os = div_up(jbgp.os, jbgp.os_block);
        jbgp.nb_os_blocking = max_divisor(jbgp.nb_os, 1, jbgp.nb_os_blocking);
...
status_t init_ip_conf_bwd_d(jit_brgemm_primitive_conf_t &jbgp) {
    ...
    jbgp.nb_os_blocking = max_divisor(jbgp.nb_os, 1, os_blocking_max);
    ...
    jbgp.nb_oc_blocking = max_divisor(jbgp.nb_oc, 1, oc_chunk_max_size);
    ...
    if ((num_work_to_parallel < 2 * jbgp.nthr
        ...
        const int max_nthr_oc_b = nstl::min(
                nstl::min((!is_amx_bf16 && !jbgp.is_bf32 && jbgp.oc > 32000)
                                ? jbgp.nthr / 2
                                : 4,
                        num_min_chunk_sz),
                jbgp.nthr);
    ...
    */
    ...
    for (int nthr_mb = 1; nthr_mb <= nthr_mb_max; ++nthr_mb) {
        ...
        if (os_chunks < nthr_mb) {
            ...
            nb_os_blocking = max_divisor(j.nb_os, 1, os_blocking_max);
        }
