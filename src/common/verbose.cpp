...
std::ostream &operator<<(std::ostream &ss, const primitive_attr_t *attr) {
    ...
    if (!legacy_input_zp.has_default_values()) {
        ...
        ss << get_val_str(legacy_input_zp.count_) << ":" << get_val_str(legacy_input_zp.mask_);
        ss << " ";
    }
    const auto &legacy_output_compensations = attr->output_compensations_;
    if (!legacy_output_compensations.has_default_values()) {
        ss << "attr-legacy-output-comp:";
        ss << get_val_str(legacy_output_compensations.count_) << ":" << get_val_str(legacy_output_compensations.mask_);
        ss << " ";
    }
